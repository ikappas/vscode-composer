...
# Ignore xdebug when executing composer
RUN mv /usr/local/bin/composer /usr/local/bin/composer.phar \
    && echo '#!/bin/bash' > /usr/local/bin/composer \
    && echo "/usr/local/bin/php -dxdebug.mode=off -dxdebug.start_with_request=no /usr/local/bin/composer.phar \$@" >> /usr/local/bin/composer \
...
RUN su vscode -c "source /usr/local/share/nvm/nvm.sh && npm install -g vsce" 2>1
